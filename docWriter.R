...
createDocPage <- function(con = stdout(), family, functions, path = "C:/R/shinyBS") {
  ...
  writeArgs <- function(args) {
    ...
    template <- readLines("templates/parameter.txt")
    op <- c()
  ...
  writeName <- function(Rd) {
    gsub("_", " ", unlist(Rd$name), fixed = TRUE)
  }
  ...
  component_template <- readLines("templates/component.txt")
  ...
  template <- readLines("templates/document.txt")
  ...
  appname <- gsub("_", "", family, fixed = TRUE)
  appname <- gsub(" ", "", family, fixed = TRUE)
  template <- gsub("<%=appname %>", appname, template, fixed = TRUE)
